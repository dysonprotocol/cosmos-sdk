...
func (k Keeper) PoolSwap(ctx context.Context, msg *whaleswapv1.MsgPoolSwap) (*whaleswapv1.MsgPoolSwapResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("invalid trader: %w", err)
	}
	...
	if strings.TrimSpace(msg.RouteIds) == "" {
		return nil, cosmossdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "route_ids required")
	}
	...
	var route []uint64
	for _, tok := range strings.Fields(msg.RouteIds) {
		pid, perr := strconv.ParseUint(tok, 10, 64)
		if perr != nil {
			return nil, cosmossdkerrors.Wrapf(sdkerrors.ErrInvalidRequest, "invalid route id: %s", tok)
		}
		route = append(route, pid)
	}
	if len(route) == 0 {
		return nil, cosmossdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "at least one pool id required")
	}
	...
	for _, pid := range route {
		pool, gerr := k.PoolsMap.Get(ctx, pid)
		if gerr != nil {
			return nil, gerr
		}
		...
		if currentDenom != pool.CoinA.Denom && currentDenom != pool.CoinB.Denom {
			return nil, cosmossdkerrors.Wrapf(sdkerrors.ErrInvalidRequest, "input denom %s not in pool %d", currentDenom, pid)
		}
		...
		if len(pool.MinPrice) == 2 {
			...
			if currentDenom == pool.CoinA.Denom {
				...
				outAmt = outDec.TruncateInt()
				if !outAmt.IsPositive() {
					return nil, fmt.Errorf("swap output too small")
				}
				pool.CoinB.Amount = pool.CoinB.Amount.Add(currentAmount)
				pool.CoinA.Amount = pool.CoinA.Amount.Sub(outAmt)
				outDenom = pool.CoinA.Denom
