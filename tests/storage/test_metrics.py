...
def test_metrics_empty_address(chainnet, generate_account):
    ...
    dysond = chainnet[0]
    [test_name, test_address] = generate_account('metrics_test', faucet_amount=1_000_000)
...
def test_metrics_single_storage_entry(chainnet, generate_account):
    ...
    dysond = chainnet[0]
    [test_name, test_address] = generate_account('metrics_test', faucet_amount=2_000_000)
    
    # Get a validator to delegate to for stake validation
    validators_result = dysond("query", "staking", "validators")
    assert len(validators_result["validators"]) > 0, "Need at least one validator for delegation test"
    ...
    delegate_result = dysond("tx", "staking", "delegate", validator_addr, delegation_amount, 
                           "--from", test_name, "--gas", "auto")
    
    assert isinstance(delegate_result, dict), f"Expected dict response, got {type(delegate_result)}: {delegate_result}"
    assert "code" in delegate_result, f"Missing 'code' field in response: {delegate_result}"
    assert delegate_result["code"] == 0, f"Delegation should succeed: {delegate_result.get('raw_log', 'No raw_log')}"
