...
def test_storage_metrics_new_structure(chainnet):
    ...
    assert "owner" in metrics_result, f"Expected 'owner' field in response: {metrics_result}"
    assert "total_bytes" in metrics_result, f"Expected 'total_bytes' field in response: {metrics_result}"
    assert "min_stake_amount" in metrics_result, f"Expected 'min_stake_amount' field in response: {metrics_result}"
    assert "current_stake_amount" in metrics_result, f"Expected 'current_stake_amount' field in response: {metrics_result}"
    
    # Verify that old nested 'metrics' field is NOT present
    assert "metrics" not in metrics_result, f"Old 'metrics' field should not be present: {metrics_result}"
    
    # Verify owner matches the queried address  
    assert metrics_result["owner"] == test_addr, f"Expected owner to be {test_addr}, got {metrics_result['owner']}"
    ...
    assert min_stake_amount == 0, f"Expected min_stake_amount ({min_stake_amount}) to be 0 with default multiplier 0"
    ...
    assert current_stake >= 0, f"current_stake_amount should be non-negative, got {current_stake}"
...
def test_storage_metrics_min_stake_calculation(chainnet, generate_account):
    ...
    storage_result = dysond("tx", "storage", "set", 
                          "--index", "test_min_stake_calculation", "--data", test_data, 
                          "--from", test_account_name, "--gas", "auto")
    
    assert isinstance(storage_result, dict), f"Expected dict response, got {type(storage_result)}: {storage_result}"
    assert storage_result["code"] == 0, f"Storage set should succeed: {storage_result.get('raw_log', 'No raw_log')}"
    ...
    assert metrics_result["min_stake_amount"] == "0"  # 0 multiplier means 0 stake required
    assert metrics_result["current_stake_amount"] == "0"  # Account should have no delegations
...
def test_storage_metrics_with_different_stake_multiple(chainnet):
    ...
    # Default should be "0"
    current_multiple = params_result["params"]["storage_stake_multiple"]
    assert current_multiple == "0", f"Expected default storage_stake_multiple to be '0', got '{current_multiple}'"
    
    print("✅ storage_stake_multiple parameter is present and properly configured")


def test_storage_metrics_response_consistency(chainnet):
    """Test that the QueryMetrics response is consistent and well-formed."""
    dysond = chainnet[0]
    
    test_account = "alice"
    alice_addr = dysond("keys", "show", test_account, "-a").strip()
    
    # Store data with known size
    test_data = "x" * 500  # 500 bytes
    storage_result = dysond("tx", "storage", "set", 
                          "--index", "consistency_test", "--data", test_data, 
                          "--from", test_account)
    
    assert storage_result["code"] == 0, f"Storage set should succeed: {storage_result['raw_log']}"
    
    # Query metrics multiple times to ensure consistency
    for i in range(3):
        metrics_result = dysond("query", "storage", "metrics", alice_addr)
        
        # Verify all required fields are present and correctly typed
        assert isinstance(metrics_result["owner"], str)
        assert isinstance(metrics_result["total_bytes"], (str, int))
        ...
        # With default storage_stake_multiple of "0", min_stake_amount should be 0
        assert min_stake_amount == 0, f"min_stake_amount {min_stake_amount} should be 0 with 0 multiplier (total_bytes: {total_bytes})"
        
        print(f"✅ Iteration {i+1}: metrics consistent")
    
    print("✅ All QueryMetrics responses are consistent") 