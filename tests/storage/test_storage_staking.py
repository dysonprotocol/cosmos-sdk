...
class TestStorageStakingParameters:
    ...
    def test_default_parameters(self, chainnet):
        ...
        print(f"✅ Default params: max_storage_size={actual_max_storage_size}, storage_stake_multiple='{actual_storage_stake_multiple}'")
...
class TestStorageStakingMetrics:
    ...
    def test_metrics_structure_and_fields(self, chainnet):
        ...
        required_fields = ["owner", "total_bytes", "min_stake_amount", "current_stake_amount"]
        for field in required_fields:
    ...
    def test_current_stake_amount_integration(self, chainnet, generate_account, faucet):
        ...
        # Create test account
        test_account_name, test_addr = generate_account('stake_metrics_test', faucet_amount=10_000_000)
        ...
        delegate_result = dysond("tx", "staking", "delegate", validator_addr, delegation_amount, 
                               "--from", test_account_name, "--gas", "auto")
    ...
    def test_multiple_delegations_sum(self, chainnet, generate_account, faucet):
        ...
        test_account_name, test_addr = generate_account('multi_delegation_test', faucet_amount=20_000_000)
        
        # Get validators
        validators = dysond("query", "staking", "validators")["validators"]
        validator1 = validators[0]["operator_address"]
        validator2 = validators[1]["operator_address"] if len(validators) > 1 else validator1
        
        # Delegate to multiple validators
        delegate1_result = dysond("tx", "staking", "delegate", validator1, "3000000udys", "--from", test_account_name)
        assert delegate1_result["code"] == 0
        
        delegate2_result = dysond("tx", "staking", "delegate", validator2, "2000000udys", "--from", test_account_name)
    ...
    def test_no_delegations_zero_stake(self, chainnet, generate_account, faucet):
        ...
        test_account_name, test_addr = generate_account('no_delegations_test', faucet_amount=1_000_000)
...
class TestStorageStakingValidation:
    ...
    def test_default_zero_multiplier_bypasses_validation(self, chainnet):
        ...
        storage_result = dysond("tx", "storage", "set",
              "--index", "test_zero_multiplier_bypass", 
              "--data", large_data,
              "--from", test_account,
              "--gas", "auto")
    ...
    def test_governance_enable_stake_validation_comprehensive(self, chainnet, generate_account, faucet):
        """Test complete stake validation functionality by enabling it via governance."""
        dysond = chainnet[0]
        
        # Create test accounts
        [insufficient_user, insufficient_addr] = generate_account('insufficient', faucet_amount=1_000_000)
        [sufficient_user, sufficient_addr] = generate_account('sufficient', faucet_amount=10_000_000)
        ...
        proposal_id = self._submit_governance_proposal_with_multiplier(dysond, "alice", "1.0")
        ...
        with pytest.raises(Exception, match="insufficient stake"):
            dysond("tx", "storage", "set",
                  ...
                  "--from", insufficient_user,
                  "--gas", "auto")
        print("✅ Storage correctly rejected for insufficient stake")
        
        # Test sufficient stake scenario
        delegate_sufficient_result = dysond("tx", "staking", "delegate", validator_operator, "2000000udys", "--from", sufficient_user)
        assert delegate_sufficient_result["code"] == 0
        
        sufficient_data = "b" * 1000  # 1000 bytes = 1000 udys required
        sufficient_result = dysond("tx", "storage", "set",
                                 "--index", "test_sufficient_stake",
                                 "--data", sufficient_data,
                                 "--from", sufficient_user,
                                 "--gas", "auto")
        ...
        assert min_stake == total_bytes, f"With 1.0 multiplier, min_stake ({min_stake}) should equal total_bytes ({total_bytes})"
        assert current_stake >= min_stake, f"Current stake ({current_stake}) should be >= min_stake ({min_stake})"
        print(f"✅ Metrics calculation: total_bytes={total_bytes}, min_stake={min_stake}, current_stake={current_stake}")
        ...
        update_result = dysond("tx", "storage", "set",
                             "--index", "test_sufficient_stake",  # Update existing
                             "--data", larger_data,
                             "--from", sufficient_user,
                             "--gas", "auto")
    ...
    def test_fractional_multiplier_calculations(self, chainnet, generate_account, faucet):
        """Test stake calculations with fractional storage_stake_multiple values."""
        dysond = chainnet[0]
        
        test_account_name, test_addr = generate_account('fractional_test', faucet_amount=10_000_000)
        ...
        delegate_test_result = dysond("tx", "staking", "delegate", validator_operator, "1000000udys", "--from", test_account_name)
        assert delegate_test_result["code"] == 0
        
        # Store 1000 bytes (should require 500 udys with 0.5 multiplier)
        test_data = "a" * 1000
        storage_result = dysond("tx", "storage", "set",
                              "--index", "fractional_test",
                              "--data", test_data,
    ...
    def test_zero_bytes_storage_always_succeeds(self, chainnet, generate_account, faucet):
        """Test that zero bytes storage always succeeds regardless of stake validation."""
        dysond = chainnet[0]
        
        test_account_name, test_addr = generate_account('zero_bytes_test', faucet_amount=1_000_000)
        ...
        empty_result = dysond("tx", "storage", "set",
                            "--index", "zero_bytes_test",
                            "--data", "",
                            "--from", test_account_name,
                            "--gas", "auto")
    ...
    def test_edge_case_large_storage_requirements(self, chainnet, generate_account, faucet):
        """Test edge cases with large storage and stake requirements."""
        dysond = chainnet[0]
        
        test_account_name, test_addr = generate_account('large_storage_test', faucet_amount=100_000_000)
        ...
        large_delegate_result = dysond("tx", "staking", "delegate", validator_operator, "50000000udys", "--from", test_account_name)
        ...
        max_data = "a" * max_size
        max_storage_result = dysond("tx", "storage", "set",
                                  "--index", "max_size_test",
                                  "--data", max_data,
                                  "--from", test_account_name,
                                  "--gas", "auto")
        ...
        print(f"✅ Large storage: {total_bytes} bytes requires {min_stake} udys, have {current_stake} udys")
        
        # Restore default
        self._submit_governance_proposal_with_multiplier(dysond, "alice", "0")

    def _submit_governance_proposal_with_multiplier(self, dysond, proposer, multiplier_value):
        ...
        # Vote on proposal
        vote_result = dysond("tx", "gov", "vote", proposal_id, "yes", "--from", proposer)
...
class TestStorageStakingErrorHandling:
    ...
    def test_invalid_address_handling(self, chainnet):
        ...
        assert "invalid" in error_message or "bech32" in error_message
    ...
    def test_nonexistent_storage_entry_handling(self, chainnet):
        ...
        result = dysond("query", "storage", "get", alice_addr, "--index", "non_existent_index")
        
        # Should return string with error information
        assert isinstance(result, str)
        error_message = result.lower()
        assert "not found" in error_message or "doesn't exist" in error_message or "key not found" in error_message
