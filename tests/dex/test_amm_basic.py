...
def test_amm_create_join_swap_exit(
    ...
    tx_create = _script_exec(
        dysond, maker_addr, maker_name, "create_pool", kwargs=create_kwargs
    )
    ...
    # Join pool with non-proportional amounts to trigger refund
    ...
    tx_join = _script_exec(
        dysond, maker_addr, maker_name, "join_pool", kwargs=join_kwargs
    )
