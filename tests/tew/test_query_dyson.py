...
'''
    ...
    response_data = next((attr["value"] for attr in exec_event["attributes"] if attr["key"] == "response"), None)
    assert response_data, f"No response found in EventExecScript: {exec_event}"
    
    submit_result = json.loads(json.loads(response_data).get("result", "{}")).get("result")
    ...
    assert response_data["type"] == "query_response", f"Unexpected response type: {response_data}"
    ...
    assert stored_data == test_data, f"Retrieved data doesn't match. Expected: {test_data}, Got: {stored_data}"
