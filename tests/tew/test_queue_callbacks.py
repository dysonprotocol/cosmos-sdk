...
def test_queue_callbacks_receive_messages_correctly(chainnet, generate_account, faucet):
    ...
    message_callbacks = [line for line in stdout1.split('\n') if 'Processing queue message' in line]
    assert len(message_callbacks) == 2, f"Expected 2 message callbacks, found {len(message_callbacks)}"
    ...
    print(f"L2 outgoing_queue: {current_state.get('l2_queue_state', {}).get('outgoing_queue', {})}")
    print(f"L2 response_queue: {current_state.get('l2_queue_state', {}).get('response_queue', {})}")
...
"""
    ...
    all_stdout3 = stdout3
    message_callbacks = [line for line in all_stdout3.split('\n') if 'Processing queue message' in line]
    response_callbacks = [line for line in all_stdout3.split('\n') if 'Processing queue response' in line]
