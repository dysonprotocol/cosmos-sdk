...
def test_submit_l2_block_basic(chainnet, generate_account, faucet):
    ...
    # Create accounts
    l1_operator_name, l1_operator_addr = generate_account("l1_op", faucet_amount=1000000)
    submitter_name, submitter_addr = generate_account("submitter", faucet_amount=1000000)
    ...
    init_result = dysond_bin(
        ...
        "--args", json.dumps(["test_instance"]),
        "--from", l1_operator_name,
        "--gas", "auto",
        "--gas-adjustment", "1.5"
    )
    assert init_result.get("code", 1) == 0
    ...
    greeting_result = dysond_bin(
        ...
        "--args", json.dumps(["Hello from L1!", "test_instance"]),
        "--from", l1_operator_name,
        "--gas", "auto",
        "--gas-adjustment", "1.5"
    )
    assert greeting_result.get("code", 1) == 0
    print("✓ L1 sent message to L2")
    
    # Verify L1 state has the message
    l1_state_result = dysond_bin(
        "query", "script", "run",
        "--executor-address", l1_operator_addr,
        "--script-address", l1_operator_addr,
        "--function-name", "get_stored_responses",
        "--args", json.dumps(["test_instance"])
    )
    ...
    storage_result = dysond_bin(
        "query", "storage", "get",
        l1_operator_addr,
        "--index", "tew/test_instance/l1"
    )
    
    # Check if storage exists - it returns a dict with 'entry' if it exists, or a string error if not
    assert isinstance(storage_result, dict), f"L1 storage should exist after say_hi, got: {storage_result}"
    assert "entry" in storage_result, f"Expected 'entry' field in storage result: {storage_result}"
    ...
    assert l1_data["queue_metadata"]["next_l2_message_id"] == 0, "Should expect message 0 from L2"
    ...
    genesis_block_data = {
        ...
        "pre_state": {
            ...
            "l1_queue_state": {
                ...
                "outgoing_queue": {"0": {"msg_id": 0, "message_data": f"{l1_operator_addr} says hi: Hello from L1!"}},
                "response_queue": {},
                "stored_responses": {}
            },
            "l2_queue_state": None
    ...
    print(f"L2 response queue: {json.dumps(l2_state.get('response_queue', {}), indent=2)}")
    assert len(l2_state["response_queue"]) == 1, f"L2 should have generated a response, block_result: {json.dumps(block_result, indent=2)}"
    ...
    submit_result = dysond_bin(
        "tx", "script", "exec",
        "--script-address", l1_operator_addr,
        "--function-name", "submit_l2_block",
        "--args", json.dumps([json.dumps(signed_l2_block), "test_instance"]),
        "--from", submitter_name,  # Different account submitting!
        "--gas", "auto",
        "--gas-adjustment", "1.5"
    )
    assert submit_result.get("code", 1) == 0, f"Failed to submit L2 block: {submit_result}"
    
    # Extract result from events
    exec_events = [e for e in submit_result.get("events", []) if e["type"] == "dysonprotocol.script.v1.EventExecScript"]
    assert len(exec_events) > 0, f"No EventExecScript found in events: {submit_result.get('events', [])}"
    
    # The response attribute is always the second one in EventExecScript
    response_attr = exec_events[0]["attributes"][1]["value"]
    ...
    assert isinstance(final_storage_result, dict), f"L1 storage should exist after submit_l2_block, got: {final_storage_result}"
    assert "entry" in final_storage_result, f"Expected 'entry' field in storage result: {final_storage_result}"
    ...
    assert len(final_l1_data["outgoing_queue"]) == 0, "Message should be removed after response"
    assert final_l1_data["queue_metadata"]["next_l2_message_id"] == 0, "Should still expect message 0 from L2 (no new messages sent)"
...
def test_submit_l2_block_sequence_enforcement(chainnet, generate_account, faucet):
    ...
    l2_state_with_gap = {
        ...
        "outgoing_queue": {
            "1": {"msg_id": 1, "message_data": "Second L2 message"}  # Skipping message 0
        },
        "response_queue": {},
        "current_height": 0,
        "processed_messages": {}
    ...
    submit_result = dysond_bin(
        ...
        "--script-address", l1_operator_addr,
        "--function-name", "submit_l2_block",
        "--args", json.dumps([json.dumps(signed_l2_block), "seq_test"]),
        "--from", l1_operator_name,
        "--gas", "auto",
        "--gas-adjustment", "1.5"
    )
    ...
    new_l2_state["outgoing_queue"]["0"] = {"msg_id": 0, "message_data": "First L2 message"}
    ...
    block2_data = {
        ...
        "tx_results": [],
        "post_state": {
            **block_result["post_state"],
            "l2_queue_state": new_l2_state
        }
    }
    ...
    submit_result2 = dysond_bin(
        ...
        "--script-address", l1_operator_addr,
        "--function-name", "submit_l2_block",
        "--args", json.dumps([json.dumps(signed_l2_block2), "seq_test"]),
        "--from", l1_operator_name,
        "--gas", "auto",
        "--gas-adjustment", "1.5"
    )
    ...
    # Extract result from events
    exec_events = [e for e in submit_result2.get("events", []) if e["type"] == "dysonprotocol.script.v1.EventExecScript"]
    assert len(exec_events) > 0, f"No EventExecScript found in events: {submit_result2.get('events', [])}"
    ...
    storage_result = dysond_bin(
        "query", "storage", "get",
        l1_operator_addr,
        "--index", "tew/seq_test/l1"
    )
    
    # Check if storage exists
    assert isinstance(storage_result, dict), f"L1 storage should exist after processing blocks, got: {storage_result}"
    assert "entry" in storage_result, f"Expected 'entry' field in storage result: {storage_result}"
    
    l1_data = json.loads(storage_result["entry"]["data"])
    print(f"L1 next_l2_message_id: {l1_data['queue_metadata']['next_l2_message_id']}")
    assert l1_data["queue_metadata"]["next_l2_message_id"] == 2, "L1 should have processed messages 0 and 1"
...
def test_multiple_submitters(chainnet, generate_account, faucet):
    ...
    l2_state = {
        ...
        "outgoing_queue": {
            "0": {"msg_id": 0, "message_data": "Message from L2"}
        },
        "response_queue": {},
        "current_height": 0,
        "processed_messages": {}
    ...
    submit1_result = dysond_bin(
        ...
        "--script-address", l1_operator_addr,
        "--function-name", "submit_l2_block",
        "--args", json.dumps([json.dumps(signed_l2_block), "multi_test"]),
        "--from", submitter1_name,
        "--gas", "auto",
        "--gas-adjustment", "1.5"
    )
    ...
    # Extract result from events
    exec_events = [e for e in submit2_result.get("events", []) if e["type"] == "dysonprotocol.script.v1.EventExecScript"]
    assert len(exec_events) > 0, f"No EventExecScript found in events: {submit2_result.get('events', [])}"
    ...
    assert "not processed" in result, "Second submission should not reprocess same messages"
