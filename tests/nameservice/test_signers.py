...
def test_destination_signs_save_class_and_mint_coins(chainnet, generate_account, faucet, register_name):
    dysond_bin = chainnet[0]

    owner_name, owner_addr = generate_account("owner_signer")
    dest_name, dest_addr = generate_account("dest_signer")
    ...
    assert resp_owner_save["code"] != 0, "owner should not be able to save-class when destination enforced"
    ...
    amount = f"10{class_id}"
    resp_owner_mint = dysond_bin(
        "tx", "nameservice", "mint-coins", "--amount", amount, "--from", owner_name
    )
    assert resp_owner_mint["code"] != 0, "owner should not be able to mint-coins when destination enforced"

    resp_dest_mint = dysond_bin(
        "tx", "nameservice", "mint-coins", "--amount", amount, "--from", dest_name
    )
    assert resp_dest_mint["code"] == 0, resp_dest_mint.get("raw_log")


def test_destination_signs_mint_nft_and_set_metadata(chainnet, generate_account, faucet, register_name):
    dysond_bin = chainnet[0]
    ...
    assert dysond_bin("tx", "nameservice", "save-class", "--class-id", class_id, "--from", dest_name)[
        "code"
    ] == 0
    ...
    assert resp_owner_mint["code"] != 0, "owner should not be able to mint-nft when destination enforced"
    ...
    assert resp_owner_meta["code"] != 0, "owner should not be able to set-nft-metadata when destination enforced"
...
def test_owner_only_set_destination_rejects_destination_signer(chainnet, generate_account, faucet, register_name):
    dysond_bin = chainnet[0]
    ...
    ok = dysond_bin(
        "tx", "nameservice", "set-destination", "--name", name, "--destination", dest_addr, "--from", owner_name
    )
    assert ok["code"] == 0, ok.get("raw_log")

    # Destination cannot set destination (should remain owner-only)
    fail = dysond_bin(
        "tx", "nameservice", "set-destination", "--name", name, "--destination", owner_addr, "--from", dest_name
    )
