...
def mint_custom_coins_with_fee_verification(dysond_bin, owner_name, owner_address, denom, amount="100"):
    ...
    assert final_community_pool >= initial_community_pool + expected_fee, \
...
def test_single_coin_minting_fee(chainnet, generate_account, faucet, register_name):
    ...
    mint_custom_coins_with_fee_verification(dysond_bin, owner_name, owner_address, denom, amount="100")
...
def test_insufficient_udys_balance_error(chainnet, generate_account, faucet, register_name):
    ...
    has_insufficient_funds_error = any(keyword in raw_log for keyword in insufficient_funds_keywords)
    
    assert mint_resp["code"] != 0 or not has_insufficient_funds_error, \
...
def test_minting_fee_event_verification(chainnet, generate_account, faucet, register_name):
    ...
    coins_minted_events = [e for e in events if e["type"] == "dysonprotocol.nameservice.v1.EventCoinsMinted"]
    ...
    has_coins_minted_event = any("coins" in event_type.lower() and "mint" in event_type.lower() 
...
def test_multiple_sequential_mints(chainnet, generate_account, faucet, register_name):
    ...
    [owner_name, owner_address] = generate_account("owner")
    faucet(owner_address, denom="udys", amount="50000000")  # Plenty for multiple mints
    ...
    min_expected_fee = 3
    assert final_community_pool >= initial_community_pool + min_expected_fee, \
    ...
    print(f"✓ Sequential mints: 3 mints completed, {final_community_pool - initial_community_pool} udys collected in fees")
