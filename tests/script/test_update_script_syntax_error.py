...
"""
    ...
    assert "txhash" in tx_broadcast, f"Expected txhash in broadcast response. Full: {json.dumps(tx_broadcast, indent=2)}"

    # Wait for inclusion with generous timeout to ensure JSON
    wait = dysond_bin("query", "wait-tx", tx_broadcast["txhash"], "--timeout", "100s")
    assert wait.get("code", 0) != 0, f"Expected failure for invalid code. Full: {json.dumps(wait, indent=2)}"
    assert "failed to format code" in wait.get("raw_log", ""), f"Missing formatting error. Full: {json.dumps(wait, indent=2)}"
