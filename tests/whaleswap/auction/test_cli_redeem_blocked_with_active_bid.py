...
def test_redeem_blocked_with_active_bid(
    chainnet, generate_account, faucet, register_name
):
    dysond = chainnet[0]
    [seller_name, seller_addr] = generate_account("auc_active_seller")
    faucet(seller_addr, amount=2_000_000)
    [bidder_name, bidder_addr] = generate_account("auc_active_bidder")
    ...
    # Redeem should be blocked while current_bidder is set
    redeem = dysond(
        "tx",
        "whaleswap",
        "redeem-auction",
