...
def test_redeem_auction_no_bidder_owner_only(
    ...
    # Redeem by seller (no bidder path)
    redeem = dysond(
        "tx",
        "whaleswap",
        "redeem-auction",
    ...
    # After redeem, auction query should fail (not found) or return empty
    q = dysond("query", "whaleswap", "auction", str(auction_id))
    assert not q.get(
        "auction"
    ), f"auction still exists after redeem: {json.dumps(q, indent=2)}"
