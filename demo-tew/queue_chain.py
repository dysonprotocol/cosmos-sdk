...
class Queueable:
    ...
    def send_message(self, msg: str):
        ...
        self.outgoing_queue[str(message_id)] = {"msg_id": message_id, "message_data": msg}
        return message_id
    ...
    def process_incoming_message(self, message: Message):
        ...
        self.response_queue[str(message["msg_id"])] = {"msg_id": message["msg_id"], "response_data": result}
    ...
    def _load_data(self, query_height: Optional[int] = None, instance_id: str = "default") -> Optional[Dict]:
        ...
        return json.loads(response["entry"]["data"]) if "entry" in response and "data" in response["entry"] else None
...
class L1(Queueable):
    ...
    def on_response(self, response: Response):
        ...
        """
        print(f"L1: Received response for message {response['msg_id']}: {response['response_data']}")
        # Store the response
    ...
    def on_l2_block(self, l2_block_data: Dict) -> bool:
        ...
        while str(next_l2_msg_id) in l2_outgoing:
            ...
            message: Message = {"msg_id": msg_data["msg_id"], "message_data": msg_data["message_data"]}
        ...
        for msg_id_str, resp_data in l2_responses.items():
            ...
            if msg_id_str in self.outgoing_queue:
                response: Response = {"msg_id": resp_data["msg_id"], "response_data": resp_data["response_data"]}
                self.process_response(response)
...
class L2(Queueable):
    ...
    def on_message(self, message: Message):
        ...
        if isinstance(message["message_data"], str) and "says hi:" in message["message_data"]:
            ...
            if len(parts) == 2:
                ...
                response = f"L2 computed response: Hello {sender_part}, you said '{greeting}'"
    ...
    def on_response(self, response: Response):
        ...
        print(f"L2: Received response for message {response['msg_id']}: {response['response_data']}")
...
def create_l1_from_snapshot(snapshot: Optional[Dict] = None):
    ...
    for msg_id, msg_data in snapshot.get("outgoing_queue", {}).items():
        if isinstance(msg_data, dict) and "msg_id" in msg_data:
            ...
            outgoing_queue[msg_id] = {"msg_id": msg_data["msg_id"], "message_data": message_content}
    ...
    for resp_id, resp_data in snapshot.get("response_queue", {}).items():
        if isinstance(resp_data, dict) and "msg_id" in resp_data:
            ...
            response_queue[resp_id] = {"msg_id": resp_data["msg_id"], "response_data": response_content}
...
def create_l2_from_snapshot(snapshot: Optional[Dict] = None):
    ...
    if snapshot:
        ...
        for k, v in outgoing_data.items():
            ...
            outgoing_queue[key] = {"msg_id": v['msg_id'], "message_data": message_content}
        ...
        for k, v in response_data.items():
            ...
            response_queue[key] = {"msg_id": v['msg_id'], "response_data": response_content}
    else:
        # Empty defaults
        queue_metadata = {}
        outgoing_queue = {}
    ...
    if snapshot:
        ...
        if 'processed_messages' in snapshot:
            ...
            processed_messages = {int(k): v for k, v in snapshot['processed_messages'].items()}
...
@l1_function  
def initialize_l2_genesis(instance_id: str = "default", chain_id: str = "", authority: str = ""):
    ...
    if "entry" not in response or "data" not in response["entry"]:
        raise Exception(f"L1 must be initialized before L2. Run initialize_l1('{instance_id}') first.")
    ...
    genesis_block_data = {
        ...
        "pre_state": {
            "accounts_by_number": {"0": {"address": authority, "account_number": 0, "sequence": 0}},
...
def say_hi(greeting: str, instance_id: str = "default"):
    ...
    if "entry" not in response or "data" not in response["entry"]:
        raise Exception(f"L1 state not found at storage key {storage_key}. Initialize L1 first.")
...
def process_l2_responses(l2_state_json: str, instance_id: str = "default"):
    ...
    for msg_id_str, resp_data in l2_responses.items():
        ...
        if msg_id_str in l1.outgoing_queue:
            # Handle both old and new field names for compatibility
            response_content = resp_data.get("response_data", resp_data.get("response"))
            response: Response = {"msg_id": resp_data["msg_id"], "response_data": response_content}
...
def submit_l2_block(signed_l2_block_json: str, instance_id: str = "default"):
    ...
    if isinstance(l2_block_data, dict) and "signer" in l2_block_data and "data" in l2_block_data:
...
def build_next_block(
    ...
    if not isinstance(block_body, dict):
        ...
        print(f"DEBUG: block_body['data'] type is dict? {isinstance(block_body['data'], dict)}")
        if isinstance(block_body['data'], dict):
            print(f"DEBUG: block_body['data'] keys: {list(block_body['data'].keys())}")
        else:
            print(f"DEBUG: block_body['data'] is not a dict, it's: {block_body['data']}")
    ...
    pending_state = json.loads(  # deep.copy is not supported in dysvm
        json.dumps(
            pre_state, separators=(",", ":"), sort_keys=True
        ),
    )
    ...
    if l1_snapshot is None:
        raise ValueError(f"L1 state not found at tew/{instance_id}/l1. L1 must be initialized before L2 can build blocks.")
    
    l1 = create_l1_from_snapshot(l1_snapshot)
    print(f"L1 state loaded from storage, next_l2_message_id: {l1.queue_metadata.get('next_l2_message_id', 0)}")
    ...
    def _process_queue_messages():
        ...
        try:
            if l2_outgoing_count > 0:
                ...
                chain_scope["print"]("L1 processed " + str(l2_outgoing_count) + " messages from L2")
        except Exception as e:
            ...
            if len(l2.response_queue) > 0:
                ...
                for resp_id, resp in sorted(l2.response_queue.items()):
                    try:
                        ...
                        chain_scope["print"](f"Error processing L2 response {resp_id}: {e}")
                ...
                chain_scope["print"](f"L1 processed {len(processed_responses)} responses from L2")
        except Exception as e:
            # Log error but don't fail block
            ...
            if l1_outgoing_count > 0:
                ...
                for msg_id, msg in sorted(l1.outgoing_queue.items()):
                    try:
                        ...
                        chain_scope["print"](f"Error processing L1 message {msg_id}: {e}")
                
                # Don't clear messages yet - they'll be cleared when responses are seen
                chain_scope["print"](f"L2 processed {l1_outgoing_count} messages from L1")
        except Exception as e:
            # Log error but don't fail block
            ...
            if len(l1.response_queue) > 0:
                ...
                for resp_id, resp in sorted(l1.response_queue.items()):
                    try:
                        ...
                        chain_scope["print"](f"Error processing L1 response {resp_id}: {e}")
                ...
                chain_scope["print"](f"L2 processed {len(processed_responses)} responses from L1")
        except Exception as e:
            # Log error but don't fail block
...
def verify_signed_block(
    ...
    if isinstance(msg_body_data, dict):
        ...
        raise ValueError(f"Expected msg_body_data to be dict after parsing, got: {msg_body_data}")
    ...
    if not isinstance(block_body["data"], dict):
        raise ValueError(f"block_body['data'] must be a dict at this point, got: {block_body['data']}")
    
    # Try accessing metadata in a safer way
    try:
        # Extract metadata from the decoded block body
        if "metadata" not in block_body["data"]:
            raise ValueError(f"block_body['data'] missing 'metadata' field. block_body['data'] = {block_body['data']}")
        
        meta: Metadata = block_body["data"]["metadata"]
    except Exception as e:
        # Debug what went wrong
        raise ValueError(f"Failed to extract metadata. block_body['data'] = {block_body['data']}, error: {e}")
    ...
    if not isinstance(block_body["data"], dict):
        raise ValueError(f"block_body['data'] must be a dict, got: {block_body['data']}")
    
    # Validate required fields for TewBlockBodyData
    required_fields = ["metadata", "pre_state", "signed_txs", "tx_results", "post_state", "prev_signed_tew_block_hash"]
    missing_fields = []
    ...
    if missing_fields:
        raise ValueError(f"Missing required fields in block data: {missing_fields}. block_body['data'] = {block_body['data']}")
...
def validate_tx(tx: SignedTewTx, block_data: TewBlockBodyData) -> tuple[str, TewTxMsg]:
    ...
    print(f"DEBUG validate_tx: raw_msg['data'] is str = {isinstance(raw_msg['data'], str)}")
    ...
    if isinstance(data_field, str):
        ...
        if isinstance(tx_body_msg, dict) and "signer" in tx_body_msg and "data" in tx_body_msg:
            # This is the tx_body_msg wrapper, extract the actual TewTxMsgData
            tx_msg_data = tx_body_msg["data"]
            print(f"DEBUG validate_tx: Extracted tx_msg_data from wrapper = {tx_msg_data}")
        else:
    ...
    if not (isinstance(tx_msg_data, dict) and "chain_id" in tx_msg_data and "sequence" in tx_msg_data):
        print(f"DEBUG validate_tx: tx_msg_data = {tx_msg_data}")
        print(f"DEBUG validate_tx: tx_msg_data is dict = {isinstance(tx_msg_data, dict)}")
    ...
    if "chain_id" not in tx_msg["data"] or "sequence" not in tx_msg["data"] or "data" not in tx_msg["data"]:
        raise ValueError(f"tx_msg['data'] missing required fields (chain_id, sequence, data). Got: {tx_msg['data']}")
...
def verify_signed_data(data: Union[SignedTewBlock, SignedTewTx, dict[str, Any]]) -> tuple[str, dict[str, Any]]:
